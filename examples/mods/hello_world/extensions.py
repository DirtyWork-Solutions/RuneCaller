...
class MyExtension(Extension):
    def __init__(self):
        super().__init__(name="Runecaller", version="1.0", dependencies=[])
    ...
    def register(self):
        ...
        super().register()
        logger.success(f"[{self.name}] Registered successfully.")
    ...
    def activate(self):
        super().activate()
        logger.success(f"[{self.name}] Activated successfully.")
        ...
        print(f"[{self.name}] Hook results: {hook_results}")
        # Dispatch an event notifying that the extension has activated.
        dispatch("extension.activated", {"extension": self.name})

    def deactivate(self):
        super().deactivate()
        logger.success(f"[{self.name}] Deactivated successfully.")

    def on_activate_hook(self, *args, **kwargs):
        ext_name = kwargs.get("extension_name", "unknown")
        logger.debug(f"[{self.name}] on_activate_hook triggered for {ext_name}.")
        # As part of the hook, dispatch another event.
        dispatch("hook.triggered", {"message": f"Activation hook executed in {self.name}."})
        return "hook_success"
